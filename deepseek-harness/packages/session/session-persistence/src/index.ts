...
export abstract class SessionPersistence extends Service {
  ...
  /**
   * Delete one persisted session's durable log. The caller must already have
   * confirmed the session is not live — backends remove the physical artifact
   * and leave any in-memory cache to self-heal (read paths revalidate the
   * durable revision, so a stale cached source is discarded once the artifact
   * is gone). Backends that cannot remove one session's log reject with an
   * explicit error so deletion never silently leaves the log behind.
   * @param id - the persisted session to delete.
   * @param signal - optional cancellation for backend deletion work.
   * @returns `true` when a log was removed, `false` when the id had none.
   */
  deleteSession(id: SessionId, signal?: AbortSignal): Promise<boolean> {
    if (signal?.aborted === true) {
      return Promise.reject(signal.reason instanceof Error ? signal.reason : new Error('aborted'))
    }
    return Promise.reject(new Error(`this session persistence backend does not support deleting session '${id}'`))
  }
}
