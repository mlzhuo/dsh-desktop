...
export class JsonlSessionPersistence extends SessionPersistence implements PersistenceBackend<JsonlTornMarker> {
  ...
  /**
   * Delete one persisted session's log directory durably. The caller must
   * have already confirmed the session is not live: this method only removes
   * the physical artifact and leaves the coordinator cache to self-heal
   * (every read path revalidates the durable revision, so a stale cached
   * source is discarded once the file is gone).
   * @param id - the persisted session to delete.
   * @param signal - optional cancellation for the search.
   * @returns `true` when a log was removed, `false` when the id had none.
   */
  override async deleteSession(id: SessionId, signal?: AbortSignal): Promise<boolean> {
    signal?.throwIfAborted()
    await this.ensureRootEncoding()
    signal?.throwIfAborted()
    const path = await this.findLog(id, signal)
    ...
    await this.syncDirPosix(dirname(dirname(path)))
    return true
  }
