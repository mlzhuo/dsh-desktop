...
export interface IWorkspaces {
  ...
  /**
   * Delete a session durably: the host refuses live sessions, drops every
   * workspace accounting slot and the archive-set membership, and removes
   * the persisted log. The row disappears on the `host/session-removed`
   * frame; a deleted current selection falls back to the New Session view.
   * @param sessionId - session to delete.
   */
  deleteSession(sessionId: SessionId): Promise<void>
  /**
   * Archive a workspace: the stats surfaces stop counting its token usage
   * and git activity from the archive instant onward (pre-archive data is
   * retained). The workspace stays in the list with a non-null `archivedAt`.
   * @param workspaceId - workspace to archive.
   */
  archiveWorkspace(workspaceId: WorkspaceId): Promise<void>
  /**
   * Unarchive a workspace: clear the archive instant, restoring its token
   * and git contributions to every stats surface.
