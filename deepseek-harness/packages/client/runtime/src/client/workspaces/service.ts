...
export class WorkspaceRuntime implements IWorkspaces {
  ...
  /**
   * Delete a session on the host: log removal, workspace accounting cleanup,
   * and archive-set membership cleanup. The host broadcasts a
   * `host/session-removed` frame that drops the session from the local list;
   * a deleted current selection falls back to the empty state automatically
   * (the list snapshot derives `current` from surviving summaries).
   * @param sessionId - session to delete.
   * @throws when the host rejects the deletion (unknown or live session).
   */
  ...
  /**
   * Archive a workspace (see IWorkspaces.archiveWorkspace).
   * @param workspaceId - workspace to archive.
   * @throws when the host rejects the operation (unknown workspace).
   */
  ...
  /**
   * Unarchive a workspace (see IWorkspaces.unarchiveWorkspace).
