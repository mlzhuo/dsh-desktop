...
export class WorkspaceManager {
  ...
  /**
   * Delete one session on the host: log removal, workspace accounting
   * cleanup, and archive-set membership cleanup. The host broadcasts a
   * `host/session-removed` frame that drops the session from the local
   * list; the returned full archive set is installed immediately so the
   * sidebar never renders a stale archived state.
  ...
  /**
   * Archive one workspace on the host; the returned workspace view is
   * upserted so the badge and stats exclusion reflect immediately.
   * @param workspaceId - workspace to archive.
   * @returns the wire result.
   */
  async archiveWorkspace(workspaceId: WorkspaceId): Promise<RpcResult<{ workspace: WorkspaceView }>> {
    const { result } = await this.api.workspace.archiveWorkspace({ workspaceId })
    if (result.ok) this.upsert(result.value.workspace)
    return result
  }

  /**
   * Unarchive one workspace on the host; the returned workspace view is
   * upserted so the badge and stats inclusion reflect immediately.
   * @param workspaceId - workspace to unarchive.
   * @returns the wire result.
   */
  async unarchiveWorkspace(workspaceId: WorkspaceId): Promise<RpcResult<{ workspace: WorkspaceView }>> {
    const { result } = await this.api.workspace.unarchiveWorkspace({ workspaceId })
    if (result.ok) this.upsert(result.value.workspace)
    return result
  }
