...
import { TokenSwitcher } from './TokenSwitcher.tsx'
  ...
  const curatedFields = (family: 'deepseek' | 'pi-ai'): ReactNode => {
    ...
    return (
      <>
        ...
        {/* A token roster is only offered where a key write actually reaches a
            request. The deepseek profile always resolves its default
            reference; another family only when its profile names one — a
            pi-ai route authenticating natively has no reference for a saved
            token to travel through, so offering the roster there would store
            keys nothing ever reads. An environment-locked key is read-only
            the same way. */}
        {props.credentialOnly === true || keyLocked
          ? null
          : layout === 'deepseek' || stringAt(fallback, 'apiKeyEnv') !== undefined
            ? (
              <TokenSwitcher
                provider={props.provider}
                keyRef={keyRef}
                ...
                disabled={disabled}
                onActivated={() => {
                  // A stored roster token now fills the reference, so the
                  // card's "already configured" hint reflects it, and a
                  // half-typed draft must not overwrite the picked token on
                  // the next Apply.
                  setKeyDraft('')
