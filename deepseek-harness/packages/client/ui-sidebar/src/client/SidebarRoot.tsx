...
import { StatsPet } from './stats/StatsPet.tsx'
...
export function SidebarRoot({
  ...
  // The pet's full-page stats overlay lives here (this column renders the
  // whole sidebar; the overlay is a fixed layer, so the mount point is only
  // about lifecycle — open state resets when the sidebar remounts).
  ...
  return (
    ...
    >
      ...
      {/* The pet sits above the browsing region in both column states: it is
          the first thing the eye meets and its click opens the stats page. */}
      <div className={css.petSeat}>
        <StatsPet
          wide={wide}
          api={statsApi}
          t={t}
      ...
      {/* Full-page stats overlay: mounted only while open (fixed layer). */}
      {statsOpen && (
        <StatsPage
          api={statsApi}
          t={t}
