...
export function apply(ctx: ClientContext): void {
  ...
  // The connection carrier owns the typed api client; the pet/page only need
  // its `stats.daily` face. Connection is composed ahead of ui-sidebar, so
  // the service is guaranteed present at apply time.
  const connection = ctx.get('connection') as unknown as { api: { stats: StatsApiFace } }
  const statsApi = connection.api.stats
