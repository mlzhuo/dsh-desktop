  ...
  const api: ApiProxy = {
    ...
    workspace: {
      ...
      archiveWorkspace: (request) => {
        ...
        const workspace = workspaces.find(w => w.workspaceId === workspaceId)
        if (workspace === undefined) {
          return err(request, { code: 'workspace-not-found', message: `no workspace ${workspaceId}`, details: { workspaceId } })
        }
        const updated = { ...workspace, archivedAt: Date.now(), updatedAt: new Date().toISOString() }
        Object.assign(workspace, updated)
        emitHost({ type: 'host/workspace-changed', workspace: { ...updated } })
        return ok(request, { workspace: { ...updated } })
      },
      unarchiveWorkspace: (request) => {
        ...
        const updated = { ...workspace, archivedAt: null, updatedAt: new Date().toISOString() }
        Object.assign(workspace, updated)
        emitHost({ type: 'host/workspace-changed', workspace: { ...updated } })
        return ok(request, { workspace: { ...updated } })
      },
    ...
    stats: {
      daily: (request) => {
        const days = request.payload.days ?? 14
        const rows: Array<{
          date: string
          tokens: { input: number; output: number; cacheRead: number; cacheWrite: number }
          costRmb: number
          code: { commits: number; linesAdded: number; linesDeleted: number }
          commits: Array<{ repo: string; message: string; hash: string; at: number }>
        }> = []
        for (let offset = days - 1; offset >= 0; offset--) {
          ...
          rows.push({
            ...
            costRmb: 0,
            code: { commits: 0, linesAdded: 0, linesDeleted: 0 },
            commits: [],
          })
        }
        return ok(request, {
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'workspace.archiveWorkspace': return this.api.workspace.archiveWorkspace(request)
      case 'workspace.unarchiveWorkspace': return this.api.workspace.unarchiveWorkspace(request)
      ...
      case 'stats.daily': return this.api.stats.daily(request, signal)
      case 'stats.balance': return this.api.stats.balance(request, signal)
    }
  }
