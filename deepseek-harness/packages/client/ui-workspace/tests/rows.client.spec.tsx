...
describe('workspace browser rows', () => {
  ...
  it('renders an active Workspace and keeps its create action separate from toggling', () => {
    ...
    const group: GroupNode = {
      key: 'project', workspaceId: wid('project'), cwd: '/projects/project', createdAt: 0, archivedAt: null, label: 'Project',
  ...
  it('workspace row menu opens on the ellipsis, renames, and shows the danger delete row', () => {
    ...
    render(<ProjectRowItem
      ...
      actions={{ rename: onRename, delete: onDelete, toggleArchive: vi.fn(), archived: false }} t={t}
    />)
  ...
  it('workspace hover card shows a POSIX home descendant as ~ and still copies the full path', async () => {
    ...
    try {
      const group: GroupNode = {
        key: 'project', workspaceId: wid('project'), cwd: '/home/u/Documents/project', createdAt: 0, archivedAt: null, label: 'Project',
        sessionCount: 0, expanded: false, containsCurrent: false, sessions: [],
      }
  ...
  it('workspace hover card without a directory omits the path and copy action', async () => {
    ...
    try {
      const group: GroupNode = {
        key: 'project', workspaceId: wid('project'), cwd: undefined, createdAt: 0, archivedAt: null, label: 'Project',
        sessionCount: 0, expanded: false, containsCurrent: false, sessions: [],
      }
      render(<ProjectRowItem group={group} home="/home/u" onToggle={vi.fn()} onCreate={vi.fn()} t={t} />)
      fireEvent.pointerEnter(screen.getByRole('treeitem').parentElement as HTMLElement)
  ...
  it('workspace hover card leaves a Windows path verbatim', async () => {
    ...
    try {
      const group: GroupNode = {
        key: 'project', workspaceId: wid('project'), cwd: 'C:\\Users\\u\\project', createdAt: 0, archivedAt: null, label: 'Project',
        sessionCount: 0, expanded: false, containsCurrent: false, sessions: [],
      }
  ...
  it('ungrouped bucket renders no workspace menu', () => {
    const group: GroupNode = {
      key: '', workspaceId: undefined, cwd: undefined, createdAt: undefined, archivedAt: null, label: 'Ungrouped',
      sessionCount: 0, expanded: false, containsCurrent: false, sessions: [],
    }
    render(<ProjectRowItem group={group} onToggle={vi.fn()} onCreate={vi.fn()} t={t} />)
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, and archive', () => {
    ...
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRename} onFork={onFork} onArchive={onArchive} onDelete={vi.fn()} t={t} />)
