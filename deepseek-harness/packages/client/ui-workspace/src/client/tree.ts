...
/** Reserved `groupExpansion` key for the trailing Archived section toggle. */
...
interface Group {
  ...
  cwd: string | undefined
  createdAt: number | undefined
  /** Archive instant (epoch ms) or `null` for the active/ungrouped bucket. */
  archivedAt: number | null
  label: string
...
function buildGroup(
  ...
  return { key, workspaceId, cwd, createdAt, archivedAt, label, sessions }
}
...
function groupByWorkspace(
  ...
  for (const workspace of workspaces) {
    ...
    groups.push(buildGroup(
      ...
      Date.parse(workspace.createdAt), workspace.archivedAt, workspace.title, members, 'account',
    ))
  }
