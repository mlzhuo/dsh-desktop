...
import { ARCHIVED_GROUP_KEY, deriveFlat, deriveGroups, deriveSearchResults, UNGROUPED_KEY } from './tree.ts'
...
> & {
  ...
  /** Toggle a real Workspace's archive state (row menu action). */
  onToggleArchive: (workspaceId: WorkspaceId) => void
...
function SessionTree({
  ...
  onRenameRequest, onDeleteRequest, onToggleArchive, onSessionRename, onSessionArchive, onSessionDelete,
  ...
  // One group section with optional workspace-row drag wiring; archived rows
  // are pinned to the trailing section and never reorder.
  const renderGroupSection = (
    group: GroupNode,
    workspaceDragProps: { start: () => void; end: () => void } | undefined,
    ...
    <div className={clsx(css.treeBody, css.wide)}>
      ...
      >
        ...
        {activeGroups.map((group) => {
          ...
          return renderGroupSection(group, workspaceDragProps, workspaceMarker, hoverWorkspace, dropWorkspace)
        })}
        {archivedGroups.length > 0 && (
          <div className={css.archivedSection}>
            <button
              ...
              onClick={() => { setGroupExpanded(ARCHIVED_GROUP_KEY, !archivedExpanded) }}
            >
              <IconTriangleRightFill14 className={clsx(css.archivedChevron, archivedExpanded && css.archivedChevronOpen)} />
              <span>{t('archive.section', { n: archivedGroups.length })}</span>
            </button>
            {archivedExpanded
              && archivedGroups.map(group => renderGroupSection(group, undefined, null, undefined, undefined))}
          </div>
        )}
      </div>
...
export function WorkspaceBrowser({
  ...
  // Delete is destructive (log removal) and therefore dialog-gated: the menu
  // action opens a confirmation dialog, and only the confirm button commits.
  ...
  const onToggleArchive = (workspaceId: WorkspaceId) => {
    const archived = workspaces.find(workspace => workspace.workspaceId === workspaceId)?.archivedAt !== null
    const operation = archived ? unarchiveWorkspace(workspaceId) : archiveWorkspace(workspaceId)
    operation.catch((reason: unknown) => {
      console.warn(archived ? 'workspace unarchive rejected:' : 'workspace archive rejected:', reason)
    })
  }
  ...
  const confirmSessionDelete = () => {
    ...
    deleteSession(sessionDeleteTarget.sessionId).then(() => {
      // Close on the resolved call: the host/session-removed frame (and the
      // workspace/archive echoes) land before or with the response, so the
      // row is already gone when the dialog closes.
