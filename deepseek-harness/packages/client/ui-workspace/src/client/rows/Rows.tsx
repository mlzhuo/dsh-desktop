...
export function ProjectRowItem({ group, onToggle, onCreate, actions, drag, home, t }: {
  ...
  actions?: {
    rename: () => void
    delete: () => void
    /** Toggle archive state (archive when active, unarchive when archived). */
    toggleArchive: () => void
    /** True while the workspace is archived (row menu shows 取消归档). */
    archived: boolean
  } | undefined
  ...
  const workspaceMenuItems = [
    ...
    ...(actions === undefined ? [] : [{
      id: 'archive',
      label: archived ? t('unarchive.workspace') : t('archive.workspace'),
      icon: archived ? <IconRefreshOutline16 /> : <IconArchiveOutline20 size={16} />,
    }]),
  ...
  const ownRow = (
    ...
    >
      ...
      <span className={css.projectText}>
        ...
        {archived && <span className={css.archivedBadge}>{t('archive.workspace.badge')}</span>}
      </span>
        ...
        {actions !== undefined && (
          <Menu
            ...
            onSelect={(id) => {
              ...
              if (id !== 'rename' && id !== 'delete' && id !== 'archive') return
              if (id === 'rename') actions.rename()
              else if (id === 'archive') actions.toggleArchive()
              else actions.delete()
            }}
            portal
            closeOnPointerLeave
 ...
 * @param props.onDelete - delete a session by id (opens the confirmation dialog).
...
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, onDelete, drag, flat = false, t }: {
  ...
  /** Delete this session (row menu action; opens the confirmation dialog, which shows the title). */
  ...
  // confirmation dialog. Delete removes the log and every accounting slot,
  // so it opens the browser-owned destructive confirmation dialog instead.
