...
import {
  ...
  workspaceArchiveWorkspaceValueSchema,
  workspaceUnarchiveWorkspaceValueSchema,
  workspaceInsertBeforeValueSchema,
  workspaceInsertSessionBeforeValueSchema,
  workspaceListValueSchema,
  workspaceRenameValueSchema,
} from '../api/workspace.schema.ts'
import { statsDailyValueSchema, statsBalanceValueSchema } from '../api/stats.schema.ts'
...
export interface IApiClient {
  ...
  workspace: {
    ...
    archiveWorkspace(payload: RequestPayload<'workspace.archiveWorkspace'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'workspace.archiveWorkspace'>>>
    unarchiveWorkspace(payload: RequestPayload<'workspace.unarchiveWorkspace'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'workspace.unarchiveWorkspace'>>>
  }
  skills: {
  ...
  stats: {
    daily(payload: RequestPayload<'stats.daily'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'stats.daily'>>>
    balance(payload: RequestPayload<'stats.balance'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'stats.balance'>>>
  }
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'workspace.archiveWorkspace': workspaceArchiveWorkspaceValueSchema,
  'workspace.unarchiveWorkspace': workspaceUnarchiveWorkspaceValueSchema,
  ...
  'stats.daily': statsDailyValueSchema,
  'stats.balance': statsBalanceValueSchema,
}
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly workspace: IApiClient['workspace'] = {
    ...
    archiveWorkspace: (payload, signal) => this.callUnary('workspace.archiveWorkspace', payload, signal),
    unarchiveWorkspace: (payload, signal) => this.callUnary('workspace.unarchiveWorkspace', payload, signal),
  }
  ...
  readonly stats: IApiClient['stats'] = {
    daily: (payload, signal) => this.callUnary('stats.daily', payload, signal),
    balance: (payload, signal) => this.callUnary('stats.balance', payload, signal),
  }
