...
import type {
  ...
  QueuedInboxItem, SessionSummary, SettingsNamespaceView, StatsCommitSample, StatsDailyValue, StatsDay, StatsDayCode,
  SubagentAddress, JobView, ToolEventView,
...
// ── stats.daily: cross-session token usage + git code activity ──────────────

/** Default trailing window (calendar days ending today). */
const STATS_DEFAULT_DAYS = 14
/** Hard cap for the trailing window. */
const STATS_MAX_DAYS = 31
/** Commits per day kept on the wire (newest first). */
const STATS_COMMITS_PER_DAY = 12
/** Hard cap on the repositories scanned per snapshot. */
const STATS_MAX_REPOS = 24
/** Git scan freshness window (ms); rapid polls reuse the previous scan. */
const STATS_GIT_TTL_MS = 15_000
/** stdout bound for one `git log --numstat` invocation. */
const STATS_GIT_MAX_BUFFER = 32 * 1024 * 1024
/** per-invocation git timeout. */
const STATS_GIT_TIMEOUT_MS = 10_000

/**
 * Official DeepSeek list prices (RMB per million tokens), mirroring the
 * 2026-08-17 pricing update. DSH usage records carry tokens only — providers
 * never attach a bill — so the cost column is ALWAYS an estimate derived
 * from these tables and each event's timestamp (the wire flags it with
 * `costEstimated`).
 *
 * Pricing periods:
 * - Events before {@link STATS_PRICE_CHANGE_MS} (2026-08-17 00:00 Beijing):
 *   flat legacy rates ({@link STATS_LEGACY_PRICES}).
 * - Events from the change instant onward: the peak/valley scheme — peak
 *   hours are Beijing 9:00–12:00 and 14:00–18:00, off-peak is exactly half
 *   of the peak rate ({@link STATS_PEAK_PRICES} / {@link STATS_VALLEY_PRICES}).
 *
 * Cache writes carry no separate list price and are billed at the
 * cache-miss (input) rate. Unknown model ids fall back to the flash rates.
 */
const STATS_PRICE_CHANGE_MS = Date.UTC(2026, 7, 16, 16, 0, 0) // 2026-08-17T00:00+08:00

/** Default credential ref for the `deepseek-official` provider's API key. */
const DEFAULT_DEEPSEEK_API_KEY_REF = credentialRef('DEEPSEEK_API_KEY')

/** Price rows keyed by the two officially priced models. */
type StatsPriceTable = Readonly<{
  'deepseek-v4-flash': Readonly<{ input: number; cacheRead: number; output: number }>
  'deepseek-v4-pro': Readonly<{ input: number; cacheRead: number; output: number }>
}>

/** Legacy flat rates in force before the 2026-08-17 change (元/百万 tokens). */
const STATS_LEGACY_PRICES: StatsPriceTable = {
  'deepseek-v4-flash': { input: 1, cacheRead: 0.02, output: 2 },
  'deepseek-v4-pro': { input: 3, cacheRead: 0.025, output: 6 },
}

/** Peak-hour rates from the 2026-08-17 change onward (元/百万 tokens). */
const STATS_PEAK_PRICES: StatsPriceTable = {
  'deepseek-v4-flash': { input: 3, cacheRead: 0.10, output: 9 },
  'deepseek-v4-pro': { input: 9, cacheRead: 0.30, output: 27 },
}

/** Off-peak rates from the 2026-08-17 change onward: exactly half of peak. */
const STATS_VALLEY_PRICES: StatsPriceTable = {
  'deepseek-v4-flash': { input: 1.5, cacheRead: 0.05, output: 4.5 },
  'deepseek-v4-pro': { input: 4.5, cacheRead: 0.15, output: 13.5 },
}

/** One model's price row for the pricing period covering `timeMs`; unknown models fall back to flash rates. */
function statsPriceRow(
  model: string | undefined,
  timeMs: number,
): Readonly<{ input: number; cacheRead: number; output: number }> {
  const row = model === 'deepseek-v4-flash' || model === 'deepseek-v4-pro' ? model : 'deepseek-v4-flash'
  if (timeMs < STATS_PRICE_CHANGE_MS) return STATS_LEGACY_PRICES[row]
  // Beijing hour (UTC+8, no DST): peak 9:00–12:00 and 14:00–18:00.
  const hour = new Date(timeMs + 8 * 3_600_000).getUTCHours()
  const peak = (hour >= 9 && hour < 12) || (hour >= 14 && hour < 18)
  return peak ? STATS_PEAK_PRICES[row] : STATS_VALLEY_PRICES[row]
}

/** One session's incremental fold state (token usage + in-force model). */
interface StatsSessionFold {
  /** Events already folded while attached (session.events index). */
  foldedCount: number
  /** Next cold-read seq (`readFrom(fromSeq)` resumes here). */
  nextSeq: number
  /** Local-day accumulators, keyed `YYYY-MM-DD`. */
  days: Map<string, StatsDayAccum>
  /** Model id in force from the latest `request/header` fold. */
  model?: string
  /**
   * Archive cut (epoch ms) of the session's workspace, or `undefined` while
   * active. Events at/after the cut are excluded from the fold; the cut is
   * stored so a later archive/unarchive change resets and replays the fold
   * with the new boundary (pre-cut days stay untouched).
   */
  archiveCut?: number
}

/** Token+cost accumulator for one local calendar day. */
interface StatsDayAccum {
  input: number
  output: number
  cacheRead: number
  cacheWrite: number
...
/** Git scan result for one repository. */
interface StatsRepoScan {
  codeByDay: Map<string, StatsDayCode>
  samples: Map<string, StatsCommitSample[]>
}

/** `git log --numstat` record separator (a control char never appears in a subject). */
const STATS_RECORD_SEP = '\x1e'
/** Header field separator inside one record's first line. */
const STATS_FIELD_SEP = '\x1f'

/** promisified `execFile` with an explicit buffer/timeout bound. */
const execFile = promisify(execFileCallback)

/** Host-local `YYYY-MM-DD` for an epoch-ms instant. */
function statsDayKey(timeMs: number): string {
...
/** The `days` trailing window's local-day keys, oldest → newest, ending today. */
function statsWindowKeys(days: number): string[] {
  const keys: string[] = []
  const now = new Date()
  for (let offset = days - 1; offset >= 0; offset--) {
    const date = new Date(now.getFullYear(), now.getMonth(), now.getDate() - offset)
    keys.push(statsDayKey(date.getTime()))
  }
  return keys
}

/** Non-negative finite number from an unknown usage field; anything else reads 0. */
function statsTokenCount(value: unknown): number {
  return typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : 0
}

/** Fold one session event into the incremental state (header tracking + usage). */
function statsFoldEvent(state: StatsSessionFold, event: SessionEvent): void {
  // Events at/after the owning workspace's archive instant are excluded:
  // pre-archive data stays, the archive moment and later do not count.
  if (state.archiveCut !== undefined && event.time >= state.archiveCut) return
  if (event.type === 'request/header') {
    const config = (event.data as { header?: { config?: { model?: unknown } } }).header?.config
    if (typeof config?.model === 'string' && config.model.length > 0) state.model = config.model
    return
  }
  ...
  const input = statsTokenCount(tokens.inputTokens)
  const output = statsTokenCount(tokens.outputTokens)
  const cacheRead = statsTokenCount(tokens.cacheReadTokens)
  const cacheWrite = statsTokenCount(tokens.cacheWriteTokens)
  if (input === 0 && output === 0 && cacheRead === 0 && cacheWrite === 0) return
  // Official list pricing is time-of-use: the 8.17 price change splits the
  // window, and post-change events are peak/valley by Beijing hour. Cache
  // writes bill at the cache-miss input rate.
  const price = statsPriceRow(state.model, event.time)
  const costRmb = (
    (input + cacheWrite) * price.input
    + cacheRead * price.cacheRead
    + output * price.output
  ) / 1_000_000
  const key = statsDayKey(event.time)
  const day = state.days.get(key) ?? { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, costRmb: 0 }
  day.input += input
  day.output += output
...
/** Merge one session's cached days into the window totals (pruned to the window floor). */
function statsMergeDays(totals: Map<string, StatsDayAccum>, state: StatsSessionFold, floorKey: string): void {
  for (const [key, day] of state.days) {
    ...
    total.cacheWrite += day.cacheWrite
    total.costRmb += day.costRmb
  }
}

/** Git repository roots under a directory: the directory itself when it is a repo, plus bounded nested repos (node_modules and dot-dirs skipped). */
async function statsGitRootsUnder(root: string, signal?: AbortSignal, depth = 3): Promise<string[]> {
  const found: string[] = []
  const visit = async (dir: string, level: number): Promise<void> => {
    signal?.throwIfAborted()
    if (found.length >= STATS_MAX_REPOS) return
    try {
      ...
      // Unreadable or vanished directories are out of scope.
    }
  }
...
/** Run one repository's git scan with a short TTL cache. */
async function statsScanRepo(
  cache: Map<string, { at: number; scan: StatsRepoScan }>,
  title: string,
  path: string,
  windowDays: number,
  floorKey: string,
  ...
  try {
    ...
    // One extra day of margin (`--since` is a UTC cut; buckets are local),
    // then the exact local-day filter below drops the strays.
    since.setDate(since.getDate() - 1)
    const result = await execFile('git', [
      '-C', path, 'log',
      `--since=${since.toISOString()}`,
      `--pretty=format:${STATS_RECORD_SEP}%H${STATS_FIELD_SEP}%at${STATS_FIELD_SEP}%s`,
      '--numstat',
    ], { encoding: 'utf8', maxBuffer: STATS_GIT_MAX_BUFFER, timeout: STATS_GIT_TIMEOUT_MS })
    stdout = String(result.stdout)
  } catch {
    // Not a repository, git absent, or a timeout: this directory simply has
    // no code column — never fail the whole snapshot over one path.
    return undefined
  }
  const codeByDay = new Map<string, StatsDayCode>()
  const samples = new Map<string, StatsCommitSample[]>()
  for (const record of stdout.split(STATS_RECORD_SEP)) {
    signal?.throwIfAborted()
    const lines = record.split('\n')
    const header = (lines.shift() ?? '').trim()
    if (header === '') continue
    const [hash, atSeconds, subject] = header.split(STATS_FIELD_SEP)
    ...
    for (const line of lines) {
      ...
      const addedN = Number(added)
      const deletedN = Number(deleted)
      if (Number.isFinite(addedN) && addedN > 0) code.linesAdded += addedN
      if (Number.isFinite(deletedN) && deletedN > 0) code.linesDeleted += deletedN
    }
    codeByDay.set(key, code)
    const list = samples.get(key)
    const sample: StatsCommitSample = {
      repo: title,
      message: (subject ?? '').trim(),
      hash,
      at: atMs,
    }
    if (list === undefined) samples.set(key, [sample])
...
/** Host-local IANA time zone the day buckets are cut in. */
function statsHostTimeZone(): string {
  try {
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // Host-stream consumers (`api.events.host`). Session deletion disposes no
  // live Session, so the host/session-removed frame has to be broadcast to
  // every connected host stream explicitly — muxQueues only reaches mux
  // consumers, and host frames must not leak into the mux schema.
  const hostQueues = new Set<FrameQueue<RpcRequest<HostFrame>>>()
  const imageAdmissionChains = new WeakMap<Agent, Promise<void>>()
  /** stats.daily incremental per-session token folds. */
  const statsSessionFolds = new Map<string, StatsSessionFold>()
  /** stats.daily git scan cache (per repo path, TTL-bounded). */
  const statsGitCache = new Map<string, { at: number; scan: StatsRepoScan }>()
  ...
  return {
    ...
    workspace: {
      ...
      async deleteSession(request) {
        ...
        try {
          ...
          // Unknown/live are business codes; storage/durability failures
        ...
        // No live Session is disposed by deletion, so the host/session-removed
        // frame must be broadcast explicitly for the client to drop the row.
      ...
      async archiveWorkspace(request) {
        const { workspaceId } = request.payload
        const branded = brandWorkspaceId(workspaceId)
        if (ctx.workspaceRegistry.get(branded) === undefined) return workspaceNotFound(request, workspaceId)
        await ctx.workspaceRegistry.archiveWorkspace(branded)
        const workspace = ctx.workspaceRegistry.get(branded)
        return ok(request, { workspace: workspaceView(workspace as Workspace) })
      },

      async unarchiveWorkspace(request) {
        const { workspaceId } = request.payload
        const branded = brandWorkspaceId(workspaceId)
        if (ctx.workspaceRegistry.get(branded) === undefined) return workspaceNotFound(request, workspaceId)
        await ctx.workspaceRegistry.unarchiveWorkspace(branded)
        const workspace = ctx.workspaceRegistry.get(branded)
        return ok(request, { workspace: workspaceView(workspace as Workspace) })
      },
    },
    ...
    stats: {
      /**
       * Daily aggregate snapshot (see StatsApi.daily): incremental token
       * folds over every attached + persisted session log, plus git-derived
       * commit/line activity over the registered workspaces' repositories.
       * The whole computation is fail-soft — a broken session or a
       * non-repository path degrades that column, never the snapshot.
       */
      async daily(request, signal) {
        signal?.throwIfAborted()
        const days = request.payload.days === undefined
          ? STATS_DEFAULT_DAYS
          : Math.min(Math.max(request.payload.days, 1), STATS_MAX_DAYS)
        const dayKeys = statsWindowKeys(days)
        const floorKey = dayKeys[0] as string
        const totals = new Map<string, StatsDayAccum>()
        for (const key of dayKeys) totals.set(key, { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, costRmb: 0 })

        // Archive cuts: sessions owned by an archived workspace stop counting
        // from the workspace's archive instant onward. Pre-archive data stays
        // folded; a cut change (archive or unarchive) resets the session fold
        // so the new boundary replays from the logs.
        const registry = ctx.get('workspaceRegistry')
        const archiveCuts = new Map<SessionId, number>()
        if (registry !== undefined) {
          for (const workspace of registry.list()) {
            if (workspace.archivedAt === null) continue
            for (const sessionId of workspace.sessionIds) archiveCuts.set(sessionId, workspace.archivedAt)
          }
        }
        const foldState = (id: SessionId): StatsSessionFold => {
          let state = statsSessionFolds.get(id)
          if (state === undefined) {
            state = { foldedCount: 0, nextSeq: 0, days: new Map() }
            statsSessionFolds.set(id, state)
          }
          const cut = archiveCuts.get(id)
          if (state.archiveCut !== cut) {
            state.days.clear()
            state.foldedCount = 0
            state.nextSeq = 0
            if (cut === undefined) delete state.archiveCut
            else state.archiveCut = cut
          }
          return state
        }

        // Attached sessions fold their in-memory logs incrementally.
        const attached = ctx.sessions.list()
        const attachedIds = new Set(attached.map(session => session.id))
        for (const session of attached) {
          signal?.throwIfAborted()
          const state = foldState(session.id)
          if (state.foldedCount > session.events.length) state.foldedCount = 0
          for (let index = state.foldedCount; index < session.events.length; index++) {
            statsFoldEvent(state, session.events[index] as SessionEvent)
          }
          state.foldedCount = session.events.length
          if (session.seq > state.nextSeq) state.nextSeq = session.seq
          statsMergeDays(totals, state, floorKey)
        }

        // Persisted sessions fold the tail past their watermark via readFrom.
        const persistence = ctx.get('sessionPersistence')
        if (persistence !== undefined) {
          const cold = (await persistence.list(signal)).filter(meta => !attachedIds.has(meta.id))
          for (const meta of cold) {
            signal?.throwIfAborted()
            const state = foldState(meta.id)
            const { events } = await persistence.readFrom(meta.id, state.nextSeq, signal)
            let maxSeq = state.nextSeq - 1
            for (const event of events) {
              statsFoldEvent(state, event)
        ...
        // Prune the cache to the requested window so it cannot grow unbounded.
        for (const state of statsSessionFolds.values()) {
          for (const key of [...state.days.keys()]) {
            if (key < floorKey) state.days.delete(key)
          }
        }

        // Git activity over the registered workspaces (plus every session's
        // working directory — a session may run in a repo the user never
        // registered, and its commits belong to the same daily columns).
        const codeByDay = new Map<string, StatsDayCode>()
        const commitSamples = new Map<string, StatsCommitSample[]>()
        ...
        const scanDir = async (title: string, path: string, cutMs?: number): Promise<void> => {
          ...
          for (const [key, code] of scan.codeByDay) {
            const total = codeByDay.get(key) ?? { commits: 0, linesAdded: 0, linesDeleted: 0 }
            total.commits += code.commits
            total.linesAdded += code.linesAdded
            total.linesDeleted += code.linesDeleted
            codeByDay.set(key, total)
          }
          for (const [key, list] of scan.samples) {
            const acc = commitSamples.get(key)
            if (acc === undefined) commitSamples.set(key, [...list])
            else acc.push(...list)
          }
        }
        ...
        const dayRows: StatsDay[] = dayKeys.map((key) => {
          const tokens = totals.get(key) as StatsDayAccum
          const code = codeByDay.get(key) ?? { commits: 0, linesAdded: 0, linesDeleted: 0 }
          const commits = (commitSamples.get(key) ?? [])
            .sort((a, b) => b.at - a.at)
            .slice(0, STATS_COMMITS_PER_DAY)
          return {
            date: key,
            tokens: {
              input: tokens.input,
            ...
            costRmb: Math.round(tokens.costRmb * 10000) / 10000,
            code,
            commits,
          }
        })
      ...
      /**
       * Account balance of the API key backing the `deepseek-official`
       * provider (see StatsApi.balance): resolved through the same credential
       * ref the chat adapter uses (`llm-deepseek` settings `apiKeyEnv`, else
       * `DEEPSEEK_API_KEY`) and queried against the official
       * `GET {baseURL}/user/balance` endpoint. The key stays on the host.
       * Every failure mode — no credentials seam, unconfigured key, or a
       * transport/HTTP error — folds into the `balance-unavailable` business
       * code so the pet/page degrade to a quiet dash instead of an error.
       */
      async balance(request, signal) {
        const settings = ctx.get('settings')
        let ref = DEFAULT_DEEPSEEK_API_KEY_REF
        if (settings !== undefined) {
          try {
            const section = settings.get(settingsNamespace('llm-deepseek')) as { apiKeyEnv?: unknown } | undefined
            if (typeof section?.apiKeyEnv === 'string' && section.apiKeyEnv.length > 0) {
              ref = credentialRef(section.apiKeyEnv)
            }
          } catch {
            // Fall back to the default ref on any settings fault.
          }
        }
        const credentials = ctx.get('credentials')
        if (credentials === undefined) {
          return err(request, {
            ...
            message: 'balance is unavailable: no credentials provider is mounted in this deployment',
            details: {},
          })
        }
        ...
        if (hit === undefined) {
          return err(request, {
            code: 'balance-unavailable',
            message: `balance is unavailable: credential ${ref} is not configured`,
            details: {},
          })
        }
        const baseURL = process.env.DEEPSEEK_BASE_URL ?? 'https://api.deepseek.com'
        let payload: { is_available?: unknown; balance_infos?: unknown }
        try {
          signal?.throwIfAborted()
          ...
          signal?.throwIfAborted()
          if (!response.ok) {
            throw new Error(`DeepSeek balance endpoint responded HTTP ${response.status}`)
          }
          payload = await response.json() as { is_available?: unknown; balance_infos?: unknown }
        } catch (error) {
          if (signal?.aborted === true) signal.throwIfAborted()
          return err(request, {
            code: 'balance-unavailable',
            message: `balance is unavailable: ${error instanceof Error ? error.message : String(error)}`,
            details: {},
          })
        }
        const infos = Array.isArray(payload?.balance_infos) ? payload.balance_infos as Array<{
          currency?: unknown
        ...
        const balances = infos.map((info) => ({
          ...
          total: Math.max(0, Number(info.total_balance) || 0),
          granted: Math.max(0, Number(info.granted_balance) || 0),
          toppedUp: Math.max(0, Number(info.topped_up_balance) || 0),
        }))
        return ok(request, {
          available: payload?.is_available === true,
          balances,
          total: balances.reduce((sum, entry) => sum + entry.total, 0),
        })
      },
    },
