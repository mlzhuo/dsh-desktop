...
export interface WorkspaceView {
  ...
  updatedAt: string
  /**
   * Archive instant (epoch ms) or `null` while the workspace is active.
   * Archived workspaces are excluded from the stats surfaces from this
   * instant onward; every session log and accounting slot is retained, so
   * unarchiving restores the exact prior picture.
   */
...
export interface WorkspaceApi {
  ...
  /**
   * Archive one workspace: the stats surfaces stop counting its token usage
   * and git activity from the archive instant onward (pre-archive data is
   * retained). Every session log and accounting slot stays, and the
   * workspace remains in `workspace.list` with a non-null `archivedAt`.
   * An unknown workspace fails with `workspace-not-found`.
   */
  archiveWorkspace(request: RpcRequest<{ workspaceId: WorkspaceId }>):
  Promise<RpcResponse<{ workspace: WorkspaceView }>>

  /**
   * Unarchive one workspace: clear the archive instant, restoring its token
   * and git contributions to every stats surface. An unknown workspace fails
   * with `workspace-not-found`.
   */
  unarchiveWorkspace(request: RpcRequest<{ workspaceId: WorkspaceId }>):
  Promise<RpcResponse<{ workspace: WorkspaceView }>>

  /**
   * Delete one session durably: the registry refuses live sessions
   * (`session-live`), verifies the session exists (`session-not-found`),
   * drops every workspace accounting slot and the archive-set membership,
   * and removes the persisted log. The client drops the row on the
   * broadcast `host/session-removed` frame. Returns the full updated
   * archive set (same snapshot the changed frame carries).
   */
