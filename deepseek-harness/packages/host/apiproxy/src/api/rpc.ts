...
export interface RpcErrorDetailsMap {
  ...
  /** A deleteSession request named a session that is live; deletion is refused while its write path is open. */
  ...
  /** A balance query failed (no credentials seam, missing key, or provider transport/HTTP error). */
  'balance-unavailable': {}
  'internal': {}
}
