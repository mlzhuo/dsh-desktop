...
/** A deleteSession request named a live session; deletion is refused while its write path is open. */
export class WorkspaceLiveSessionError extends Error {
  /**
   * @param sessionId - The live session id.
   */
  constructor(readonly sessionId: SessionId) {
    super(`cannot delete session '${sessionId}': the session is live; stop or close it before deleting`)
...
export class WorkspaceRegistry extends Service {
  ...
  /**
   * Delete one session durably: refuse live sessions, verify the session
   * exists, drop every workspace accounting slot and the archive-set
   * membership, then remove the persisted log. The workspace updates flow
   * through `table.update` (each emits its own domain change), the archive
   * set through {@link setState}; the log removal happens last so a
   * persistence failure leaves accounting intact for a retry.
   * @param sessionId - The session to delete.
   * @returns resolution after durability.
   * @throws {WorkspaceLiveSessionError} when the session is live.
   * @throws {WorkspaceUnknownSessionError} when neither live nor persisted.
   */
    ...
    return this.enqueueOperation(async () => {
      ...
      for (const [workspaceId, record] of [...table.entries()]) {
        ...
        await table.update(workspaceId, current =>
          current.sessionIds.length === sessionIds.length ? current : {
            ...current,
            sessionIds,
            updatedAt: new Date().toISOString(),
          })
      }
      await this.ctx.sessionPersistence.deleteSession(sessionId)
  ...
  /**
   * Archive one workspace: stamp the archive instant on its durable record.
   * The workspace and every session log/accounting slot are retained, but
   * the stats surfaces exclude its token usage and git activity from the
   * archive instant onward (pre-archive data stays). Unknown ids reject.
  ...
  archiveWorkspace(workspaceId: WorkspaceId): Promise<void> {
    return this.enqueueOperation(async () => {
      ...
      if (entity === undefined) throw new Error(`cannot archive unknown workspace '${workspaceId}'`)
      await entity.archive()
    })
  }

  /**
   * Unarchive one workspace: clear the archive instant, restoring its token
   * and git contributions to every stats surface. Unknown ids reject.
   * @param workspaceId - the workspace to unarchive.
   * @returns resolution after durability.
   */
  unarchiveWorkspace(workspaceId: WorkspaceId): Promise<void> {
    return this.enqueueOperation(async () => {
      const entity = this.entities.get(workspaceId)
      if (entity === undefined) throw new Error(`cannot unarchive unknown workspace '${workspaceId}'`)
