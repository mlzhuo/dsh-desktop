...
export const workspaceRecord = z.object({
  ...
  updatedAt: z.string(),
  /**
   * Archive instant (epoch ms) when the workspace was archived, `null` when
   * active. Archived workspaces stay in the registry and keep every session
   * log and accounting slot, but are excluded from the stats surfaces from
   * this instant onward (tokens and git code both); unarchiving restores
   * the exact prior picture. Defaulted so records written before the field
   * parse unchanged.
   */
