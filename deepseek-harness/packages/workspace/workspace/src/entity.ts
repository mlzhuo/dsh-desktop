...
export class WorkspaceEntity implements Workspace {
  ...
  get archivedAt(): number | null {
    return this.record.archivedAt ?? null
  }
  ...
  /** Archive the workspace: stamp the archive instant (idempotent no-op when already archived). */
  async archive(archivedAt: number = Date.now()): Promise<void> {
    await this.mutate(record => record.archivedAt !== null ? record : { ...record, archivedAt })
  }

  /** Unarchive the workspace: clear the archive instant (idempotent no-op when active). */
  async unarchive(): Promise<void> {
    await this.mutate(record => record.archivedAt === null ? record : { ...record, archivedAt: null })
  }
